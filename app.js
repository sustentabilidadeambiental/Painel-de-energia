...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof rawDashboardData !== 'undefined') {
        ...
        // Unify consumption units: convert MWh to kWh for HSI, Pupileira, Anexo HSI, and HMS
        const unitsToConvert = ['HSI', 'Pupileira', 'Anexo HSI', 'HMS'];
        unitsToConvert.forEach(unitKey => {
            const unit = appData.unidades[unitKey];
            if (unit) {
                if (unit.orcado) {
                    unit.orcado.forEach(row => {
                        if (row.type === 'consumo' && (row.label === 'Previsão de Consumo' || row.label === 'Previsão Consumo' || row.label === 'Consumo Real')) {
                            row.values = row.values.map(val => val * 1000);
                        }
                    });
                }
                if (unit.realizado) {
                    unit.realizado.forEach(row => {
                        if (row.type === 'consumo' && (row.label === 'Previsão de Consumo' || row.label === 'Previsão Consumo' || row.label === 'Consumo Real')) {
                            row.values = row.values.map(val => val * 1000);
                        }
                    });
...
// Helper to find the last month index with realized data for a unit
function getUnitLastRealizedMonth(unit) {
    let lastMonth = 0;
    const realizedTotalRow = unit.realizado.find(r => r.label === 'TOTAL GERAL');
    if (realizedTotalRow && realizedTotalRow.values) {
        for (let m = 11; m >= 0; m--) {
            if (Math.abs(realizedTotalRow.values[m]) > 0.01) {
                return m;
            }
        }
    }
    return 6; // fallback to July
}

// Calculate and render alerts in the unit detail view
function calculateAndRenderAlerts(unit, sheetName) {
    const alertsContainer = document.getElementById('unit-alerts-container');
    if (!alertsContainer) return { maxDiffMonth: -1, maxSavingMonth: -1 };
    ...
    if (!orcadoTotalRow || !realizadoTotalRow) {
        alertsContainer.style.display = 'none';
        return { maxDiffMonth: -1, maxSavingMonth: -1 };
    }
    
    const lastRealizedMonthUnit = getUnitLastRealizedMonth(unit);
    
    // Sum for active period
    let periodOrcado = 0;
    let periodRealizado = 0;
    for (let m = startMonthIdx; m <= endMonthIdx; m++) {
        periodOrcado += Math.abs(orcadoTotalRow.values[m]);
        periodRealizado += Math.abs(realizadoTotalRow.values[m]);
    }
    
    const alerts = [];
    
    // 1. 🔴 Estouro de orçamento (Selected Period)
    if (periodRealizado > periodOrcado + 0.01) {
        const diff = periodRealizado - periodOrcado;
        const pct = diff / periodOrcado;
        alerts.push({
            icon: '🔴',
            title: 'Estouro de Orçamento',
            desc: `Realizado no período excedeu o Orçado em ${formatCurrency(diff)} (+${formatPercentage(pct)}).`,
            type: 'danger'
        });
    }
    
    // 2. 🟡 Risco de estouro anual
    // Calculate total annual orçado
    let annualOrcado = 0;
    for (let m = 0; m < 12; m++) {
        annualOrcado += Math.abs(orcadoTotalRow.values[m]);
    }
    
    // Calculate YTD realized
    let ytdRealizado = 0;
    for (let m = 0; m <= lastRealizedMonthUnit; m++) {
        ytdRealizado += Math.abs(realizadoTotalRow.values[m]);
    }
    
    // Project annual cost: (YTD Realized / months elapsed) * 12
    const monthsElapsed = lastRealizedMonthUnit + 1;
    const projectedAnnual = (ytdRealizado / monthsElapsed) * 12;
    
    if (projectedAnnual > annualOrcado + 0.01) {
        alerts.push({
            icon: '🟡',
            title: 'Risco de Estouro Anual',
            desc: `Projeção anual de ${formatCurrency(projectedAnnual)} supera o orçamento total de ${formatCurrency(annualOrcado)}.`,
            type: 'warning'
        });
    }
    
    // 3. ⚠️ Maior desvio do ano (Look at entire year up to lastRealizedMonthUnit)
    let maxDiff = -1;
    let maxDiffMonth = -1;
    let maxDiffOrcado = 0;
    let maxDiffRealizado = 0;
    for (let m = 0; m <= lastRealizedMonthUnit; m++) {
        const oVal = Math.abs(orcadoTotalRow.values[m]);
        const rVal = Math.abs(realizadoTotalRow.values[m]);
        const diff = Math.abs(rVal - oVal);
        if (diff > maxDiff) {
            maxDiff = diff;
            maxDiffMonth = m;
            maxDiffOrcado = oVal;
            maxDiffRealizado = rVal;
        }
    }
    
    if (maxDiffMonth !== -1) {
        const isOverrun = maxDiffRealizado > maxDiffOrcado;
        const typeStr = isOverrun ? 'Gasto acima' : 'Economia abaixo';
        alerts.push({
            icon: '⚠️',
            title: 'Maior Desvio do Ano',
            desc: `Mês com maior diferença: ${monthNames[maxDiffMonth]} (${typeStr} do orçado em ${formatCurrency(maxDiff)}: Realizado ${formatCurrency(maxDiffRealizado)} vs. Orçado ${formatCurrency(maxDiffOrcado)}).`,
            type: isOverrun ? 'danger' : 'warning'
        });
    }
    
    // 4. 📈 Tendência de alta (Within selected period)
    let trendDetected = false;
    for (let m = endMonthIdx; m >= startMonthIdx + 2; m--) {
        const rCurrent = Math.abs(realizadoTotalRow.values[m]);
        const rPrev1 = Math.abs(realizadoTotalRow.values[m-1]);
        const rPrev2 = Math.abs(realizadoTotalRow.values[m-2]);
        if (rCurrent > rPrev1 && rPrev1 > rPrev2) {
            alerts.push({
                icon: '📈',
                title: 'Tendência de Alta',
                desc: `Aumento consecutivo de custos por 3 meses (de ${monthNames[m-2]} a ${monthNames[m]}).`,
                type: 'danger'
            });
            trendDetected = true;
            break;
    ...
    if (!trendDetected) {
        for (let m = endMonthIdx; m >= startMonthIdx + 1; m--) {
            const rCurrent = Math.abs(realizadoTotalRow.values[m]);
            const rPrev1 = Math.abs(realizadoTotalRow.values[m-1]);
            if (rCurrent > rPrev1) {
                alerts.push({
                    icon: '📈',
                    title: 'Tendência de Alta',
                    desc: `Aumento consecutivo de custos por 2 meses (de ${monthNames[m-1]} a ${monthNames[m]}).`,
                    type: 'danger'
                });
                break;
            }
    ...
    // 5. ⚡ Consumo fora do padrão (Selected Period relative to historical average)
    const consumptionRealizadoRow = unit.realizado.find(r => r.type === 'consumo');
    const hasPhysicalCons = !!consumptionRealizadoRow;
    const valuesToCheck = hasPhysicalCons 
        ? consumptionRealizadoRow.values.map(Math.abs)
        : realizadoTotalRow.values.map(Math.abs);
        
    let sumHistorical = 0;
    let countHistorical = 0;
    for (let m = 0; m <= lastRealizedMonthUnit; m++) {
        sumHistorical += valuesToCheck[m];
        countHistorical++;
    }
    const avgHistorical = countHistorical > 0 ? (sumHistorical / countHistorical) : 0;
    ...
    for (let m = startMonthIdx; m <= endMonthIdx; m++) {
        const val = valuesToCheck[m];
        if (avgHistorical > 0) {
            const dev = (val - avgHistorical) / avgHistorical;
    ...
    if (Math.abs(maxDev) > 0.25 && maxDevMonth !== -1) {
        const isAbove = maxDev > 0;
        const typeStr = isAbove ? 'muito acima' : 'muito abaixo';
        const labelStr = hasPhysicalCons ? 'consumo' : 'custo';
        alerts.push({
            icon: '⚡',
            title: 'Consumo Fora do Padrão',
            desc: `O ${labelStr} em ${monthNames[maxDevMonth]} ficou ${formatPercentage(Math.abs(maxDev))} ${typeStr} da média histórica.`,
            type: isAbove ? 'danger' : 'success'
        });
    }
    
    // 6. 💰 Maior economia do ano
    let maxSaving = 0;
    let maxSavingMonth = -1;
    let maxSavingOrcado = 0;
    let maxSavingRealizado = 0;
    for (let m = 0; m <= lastRealizedMonthUnit; m++) {
        const oVal = Math.abs(orcadoTotalRow.values[m]);
        const rVal = Math.abs(realizadoTotalRow.values[m]);
        const saving = oVal - rVal;
        if (saving > maxSaving) {
            maxSaving = saving;
            maxSavingMonth = m;
            maxSavingOrcado = oVal;
            maxSavingRealizado = rVal;
        }
    }
    
    if (maxSavingMonth !== -1 && maxSaving > 0.01) {
        alerts.push({
            icon: '💰',
            title: 'Maior Economia do Ano',
            desc: `Mês de economia excepcional em ${monthNames[maxSavingMonth]} (Economia de ${formatCurrency(maxSaving)}).`,
            type: 'success'
        });
    }
    ...
    if (alerts.length > 0) {
        ...
        alertsContainer.style.display = 'grid';
        alertsContainer.innerHTML = `
            <div class="alert-card" style="grid-column: 1 / -1; justify-content: center; align-items: center; padding: 20px;">
                <div class="alert-card-icon" style="font-size: 1.5rem;">✅</div>
                <div class="alert-card-content" style="text-align: center;">
                    <div class="alert-card-title">Sem Alertas Ativos</div>
                    <div class="alert-card-desc">Todos os indicadores estão dentro da normalidade para esta unidade.</div>
                </div>
            </div>
        `;
    }
...
function renderUnitDashboard(sheetName) {
    ...
    // Calculate and render alerts bar
    const alertsResult = calculateAndRenderAlerts(unit, sheetName);
    ...
    for (let m = startMonthIdx; m <= endMonthIdx; m++) {
        ...
        const tr = document.createElement('tr');
        if (m === alertsResult.maxDiffMonth) {
            if (rTotal > oTotal) {
                tr.className = 'row-highlight-deviation';
            } else if (m === alertsResult.maxSavingMonth) {
                tr.className = 'row-highlight-economy';
            } else {
                tr.className = 'row-highlight-deviation';
            }
        } else if (m === alertsResult.maxSavingMonth) {
